            ...
            <div class="st-tabcon">
                <ul>
                    ...
                    <li {if $sindex%3==0}class="mr_0"{/if}>
                        <a class="pic" href="{$s['url']}" target="_blank"><img src="{Product::get_lazy_img()}" st-src="{Common::img($s['litpic'],294,200)}" alt="{$s['title']}"/></a>
                        <a class="tit" href="{$s['url']}" target="_blank">{$s['title']}</a>
                              <span class="price">
                                   {if !empty($s['price'])}
                                   <i class="currency_sy">{Currency_Tool::symbol()}</i><b>{$s['price']}</b>起
                                  {/if}
                              </span>
                    </li>
                    {php}$sindex++;{/php}
