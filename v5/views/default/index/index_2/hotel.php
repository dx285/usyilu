    ...
    <div class="product_box">
        <div class="sidle_hot">
            ...
            <ul>
                ...
                <li>
                    <s></s>
                    <div class="pic"><a href="{$h['url']}" target="_blank"><img class="lazyimg" src="{Product::get_lazy_img()}" st-src="{Common::img($h['litpic'],180,122)}" alt="{$h['title']}"></a></div>
                    <p class="tit"><a href="{$h['url']}" target="_blank">{$h['title']}</a></p>
                    <p class="jg">{if $h['price']>0}<i class="currency_sy">{Currency_Tool::symbol()}</i>{$h['price']}{else}电询{/if}</p>
                </li>
                {/loop}
                {/st}
