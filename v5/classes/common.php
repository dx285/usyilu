...
class Common
{
    ...
    * */
    ...
    {
        return St_Functions::remove_xss($param, $reject_check);
    }
...
EOT;
    ...
    {
        if (stripos($src, 'http://') !== false && stripos($src, rtrim($GLOBALS['cfg_basehost'], '/')) === false)
        {
            return $src;
        }
        ...
        else if (empty($src))
        {
            ...
            if ($width != '' || $height != '')
            {
                ...
                return strpos($nopic, 'http://') === false ? rtrim($GLOBALS['cfg_basehost'], '/') . $nopic : $nopic;
            }
            return $nopic;//无图返回
        }
        else
