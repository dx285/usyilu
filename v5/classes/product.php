...
class Product
{
    ...
    * */
    ...
    public static function get_use_templet($pagename)
    {
        ...
        {
            $sql = "SELECT b.path FROM `sline_site_page` a LEFT JOIN `sline_site_page_config` b ON a.id=b.pageid WHERE a.pagename='$pagename' AND b.isuse = 1 AND b.webid='$sys_webid'";
            $row = DB::query(1, $sql)->execute()->as_array();;
            if (isset($row[0]['path']))
            {
                $templet = 'usertpl/' . $row[0]['path'] . '/index';
