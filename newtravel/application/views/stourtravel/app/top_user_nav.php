...
<body style="overflow:hidden" top_padding=X2Lpkt >
                        ...
                        if(value=='add'){
                            ...
                            var html="<div style='vertical-align: bottom; height:30px;'>" +
                                "<span style='float:left; display: block; right:20px; width:30px; ' id='record_src_"+id+"'>"+
                                 imgHtml +
                                "</span>"+
                                "<div style='float:left;height:20px;'><div id='file_upload_"+id+"' class='btn-file mt-4'><a href='javascript:void(0);' onclick=\"uploadlitpic(" + id + ")\">"+str+"</a><a href='javascript:void(0);' style='padding: 10px;"+del_hide+"'  onclick=\"del(" + id + ")\">删除</a></div></div></div>";
                             return html;
                        }
...
function uploadlitpic(domid){
    ...
    function del(id)
    {
        ST.Util.confirmBox("提示","确定删除？",function(){
            Ext.Ajax.request({
                url: SITEURL + "app/topusernav/action/delicon/",
                method: "POST",
                datatype: "JSON",
                params: {id: id},
                success: function (response, opts) {
