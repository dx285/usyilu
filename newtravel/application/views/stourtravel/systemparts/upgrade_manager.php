...
<head>
    <meta charset="utf-8">
    <title>系统模块升级管理-思途CMS{$coreVersion}</title>
...
<scirpt id="app_upgrade" type="text/html" style="display: none">
    <p>
        <label>
            正版授权应用数：</label>
        <em>{{app_count}}个</em> <a class="manage-a" href="javascript:;"  onclick="ST.Util.addTab('我的应用','{$cmsurl}mall/app/menuid/193')"><i></i>&nbsp;<span>管理应用</span></a>
    ...
    <p>
        <label>
            已禁止更新应用数：</label>
        <em class="sj_disabled">{{app_disabled_upgrade_count}}个</em> <a class="manage-a" href="javascript:;"  onclick="ST.Util.addTab('我的应用','{$cmsurl}mall/app/menuid/193')"><i></i>&nbsp;<span>管理应用</span></a>
...
<script>
    ...
    {
        ST.Util.addTab('升级详情','{$cmsurl}upgrade/patch_list/system_part_code/'+core_system_data.system_part.code+"/menuid/192");
    }
</script>
