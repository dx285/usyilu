...
<scirpt id="get_patch_list_error" type="text/html" style="display: none;">
    ...
    {{else}}
        <p class="num">共需升级<em>{{task_count}}</em>个应用，正在升级{{current_task.name}}，第<span>{{current_task_position}}/{{task_count}}</span>个</p>
        <p class="error">获取应用{{current_task.name}}的更新包失败，原因：{{#error_msg}}</p>
    {{/if}}
</scirpt>
...
<scirpt id="patch_exec_error" type="text/html" style="display: none;">
    ...
    {{else}}
        <p class="num">共需升级<em>{{task_count}}</em>个应用，正在升级{{current_task.name}}，第<span>{{current_task_position}}/{{task_count}}</span>个</p>
        <p class="tiwen">应用{{current_task.name}}共{{patch_count}}个更新，正在执行第{{current_patch_position}}个</p>
    {{/if}}
...
<scirpt id="get_patch_list" type="text/html" style="display: none;">
    ...
    {{else}}
        <p class="num">共需升级<em>{{task_count}}</em>个应用，正在升级{{current_task.name}}，第<span>{{current_task_position}}/{{task_count}}</span>个</p>
        <p class="tiwen">获取应用{{current_task.name}}的更新包，请稍候...</p>
    {{/if}}
