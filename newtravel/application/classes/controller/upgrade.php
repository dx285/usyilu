...
class Controller_Upgrade extends Stourweb_Controller
{
    ...
    private function send_task_data()
    {
        if (!Model_Upgrade3::check_webroot_dir_write()) //检测目录写权限
        {
            $this->assign("error_msg", '没有权限操作,请打开网站目录写权限');
            return;
        }
        if (!$this->checkLicense()) //检测序列号是否正确
        {
            $this->assign("error_msg", '序列号验证失败,请检查序列号');
            return;
        }

        $ugrade_task_list = Model_Upgrade3::upgrade_task_library();
        $this->assign("task_list", json_encode($ugrade_task_list));
    }
    ...
    public function action_ajax_exec_install()
    {
        ...
        if (empty($current_task["app_number"]) || $current_task["system_part_type"] != "2")
        {
            $result["msg"] = "不是正确的应用安装任务";
            exit(json_encode($result));
        }
        ...
        {
            $result['msg'] = '应用文件安装失败,' . $app_files_install_result['errormsg'];
            exit(json_encode($result));
        }
    ...
    public function action_ajax_exec_uninstall()
    {
        ...
        {
            $result['msg'] = '应用配置文件目录删除失败，' . $app_config_install_dir_del_result["errormsg"];
            exit(json_encode($result));
        }
    ...
    public function action_ajax_exec_upgrade_patch()
    {
        ...
        if ($current_task["system_part_type"] == "2")
        {
    ...
    public function action_ajax_check_all_systempart_update()
    {
        ...
        $app_install_data_list = $appapi_model->app_install_data();
