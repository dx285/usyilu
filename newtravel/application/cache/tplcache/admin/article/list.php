...
<head>
<meta charset="utf-8">
<title>文章管理-思途CMS<?php echo $coreVersion;?></title>
...
<script>
 ...
 Ext.tip.QuickTipManager.init();
        ...
        $("#addbtn").click(function(){
            ST.Util.addTab('添加文章','<?php echo $cmsurl;?>article/admin/article/add/<?php if(isset($_GET['menuid'])) { ?>menuid/<?php echo $_GET['menuid'];?>/<?php } ?>
parentkey/article/itemid/1',0);
        });
...
//产品store
 ...
 fields:[
   ...
   type:'ajax',
   api: {
              read: SITEURL+'article/admin/article/article/action/read',  //读取数据的URL
  update:SITEURL+'article/admin/article/article/action/save',
  destroy:SITEURL+'article/admin/article/article/action/delete'
              },
      reader:{
                type: 'json',   //获取数据的格式 
                root: 'lists',
...
//实现动态窗口大小
  ...
  Ext.Ajax.request({
 url   :  SITEURL+"article/admin/article/article/action/update",
 method  :  "POST",
 datatype  :  "JSON",
 ...
 if(response.responseText=='ok')
 {
  ...
  //修改
    function goModify(id)
    {
        ...
        var url = SITEURL+'article/admin/article/edit/<?php if(isset($_GET['menuid'])) { ?>menuid/<?php echo $_GET['menuid'];?>/<?php } ?>
parentkey/article/itemid/1/id/'+id;
        parent.window.addTab('修改-'+title,url,1);
    }
  //官方攻略信息配置
    function goOffical(id,name)
    {
        var url = SITEURL+'article/admin/article/infoset/parentkey/article/itemid/1/articleid/'+id;
