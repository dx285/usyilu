...
sql;
        ...
        foreach ($listenerlist as $listener)
        {
            ...
            $execresult = self::_request($execurl);
    ...
    {
        $typeid = $params['typeid'] ? $params['typeid'] : 0;
    ...
    public static function back_order_status_changed($org_status, $order, $product_model)
    {
        ...
        if ($order['status'] == 3 && $org_status!=4)
        {
            ...
            if(St_Functions::is_normal_app_install('coupon'))
            {
                ...
                Model_Coupon::cancel_order_back($order['ordersn']);
            }
    ...
    public static function back_caculate_info_by_timerange($timearr, $typeid)
    {
         ...
         //已付款
         $arr = ORM::factory('member_order')->where("{$where} typeid=$typeid and (status=2 or status=5)")->get_all();
         $price = 0;
         foreach ($arr as $row)
         {
         ...
         //未付款
         $arr = ORM::factory('member_order')->where("{$where} typeid=$typeid and (status=0 or status=1)")->get_all();
         $price = 0;
         foreach ($arr as $row)
         {
             $price += self::order_total_price($row['id'],$row);
