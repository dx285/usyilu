...
class Model_Model extends ORM
{
    ...
    public static function get_search_model()
    {
        ...
        /* $arr = ORM::factory('model')
         ...
         return $arr;*/
        //排除7(保险),9(机票),10(问答),11(结伴),12(目的地),14(私人定制),101(游记)模块
        /* $arr = ORM::factory('nav')
             ->where("typeid not in(7,9,10,11,12,14,101)")
        ...
        $sql.= "AND a.typeid not in(7,9,10,11,12,14,101) ";
    ...
    public static function get_used_model()
    {
        ...
        foreach($arr as &$r)
        {
            if($r['typeid']>103 && $r['typeid']<200 && !self::is_install_model($r['typeid']))
            {
                continue;
            }
            $r['modulename'] = $r['shortname'];
    ...
    public function createAdminMenu($typeid, $pinyin, $modulename)
    {
        ...
        foreach ($base as $k => &$j)
        {
            $sql = "select * from sline_menu_new where typeid={$j[2]} and level={$j[1]} and title= '{$j[3]}'";
            $findresult = DB::query(DataBase::SELECT, $sql)->execute()->as_array();
