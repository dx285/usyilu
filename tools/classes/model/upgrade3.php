...
class Model_Upgrade3
{
    ...
    public static function upgrade_task_library(array $upgrade_task_list = null)
    {
        ...
        } else
        {
            ...
            foreach ($upgrade_task_list as $upgrade_task)
            {
                ...
                if ($upgrade_task["system_part_type"] == "2")
    ...
    public static $INSTALL_CONFIG_FILES = array('FileManifest.txt', 'installer.ini', 'sql.php', 'unsql.php');
    ...
    public static function setup_install_config_files($unzippath, $app_install_dir, $version = '0.0.0.0')
    {
        ...
        foreach ($files as $file)
        {
            ...
            if ($ext == 'php')
            {
                $distFile = $app_install_dir . "/{$version}.php";
            }
            if ($ext == 'txt')
            {
                $distFile = $app_install_dir . "/{$version}.txt";
            }
    ...
    //检测是否有写入权限
    public static function check_webroot_dir_write()
    {
        $flag = 0;
