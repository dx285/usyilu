...
class Product
{
    ...
    public static function strip_style($str)
    {
        ...
        $str = preg_replace('~src="(.*?\.(?:jpg|gif|png|jpeg))"~', "src=\"/phone/public/images/grey.gif\" st-src=\"\\1\"", $str);
    ...
    {
        preg_match_all('~((?:font-size|color|text-decoration)\s*:.*?;)~is', $match[1], $rs);
    ...
    public static function minus_storage($dingdate, $typeid, $suitid, $productid, $dingnum, $departdate = '')
    {
        ...
        switch ($typeid)
        {
            ...
            case '13':
                ...
                break;
            default:
                $sql = "UPDATE `sline_model_suit` SET number=number-$dingnum WHERE id='$suitid' AND number!=0 AND number!=-1";
                DB::query(Database::UPDATE, $sql)->execute();
                break;
        }
