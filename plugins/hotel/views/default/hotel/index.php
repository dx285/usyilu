...
<body>
  ...
  <div class="big">
      ...
      <div class="st-hotel-home-top">
          ...
          <script>
              $(function(){
                  ...
                  $('.st-hs-btn').click(function(){
                        ...
                        var params={keyword:$(".keyword").val(),starttime:$('#txtHotelTime1').val(),endtime:$('#txtHotelTime2').val()}
                        var query=new Array();
                        var destpy = "";
                        if(destname!=''){
          ...
          <div class="bd">
            <ul>
                {loop $hotelad['aditems'] $v}
                    <li><a href="{$v['adlink']}" target="_blank"><img src="{Product::get_lazy_img()}" original-src="{Common::img($v['adsrc'],768,324)}" /></a></li>
                {/loop}
            </ul>
          </div>
          <!--前/后按钮代码-->
        ...
        <div class="st-hotel-home-box">
          ...
          <div class="hotel-tj-box">
             ...
             {st:hotel action="query" flag="order" row="3"}
                {loop $data $row}
                 <li>
                    <div class="pic"><img src="{Product::get_lazy_img()}" st-src="{Common::img($row['litpic'],400,271)}" /></div>
                    <div class="txt">
        ...
        <div class="st-hotel-home-box">
        	<div class="hotel-tit-ico"><h3 class="jx-ico">精选{$channelname}</h3><a class="more" href="/hotels/all" target="_blank">查看更多</a></div>
          	...
          	<ul>
                {st:hotel action="query" flag="order" row="8" offset="3"}
                 ...
                 {loop $data $row}
                    ...
                    <div class="pic"><a href="{$row['url']}"><img src="{Product::get_lazy_img()}" st-src="{Common::img($row['litpic'],277,188)}" alt="{$row['title']}" /></a></div>
                    <div class="txt">
                      ...
                      <p class="num"><span>满意度：{$row['satisfyscore']}</span><span>销量：{$row['sellnum']}</span></p>
                      <p class="ads"><span>{$row['address']}</span></p>
                    </div>
                    <div class="list">

                       {st:hotel action="suit" row="2" productid="$row['id']" return="roomlist"}
                        {loop $roomlist $room}
                            <a href="{$row['url']}" target="_blank">
