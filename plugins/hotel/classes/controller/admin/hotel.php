...
,ifnull(b.displayorder,9999) as displayorder,ifnull(d.suitday,0) as suitday from sline_hotel as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=2) left join (select c.hotelid,c.id,min(c.suitday) as suitday from(select a.hotelid,a.id,max(ifnull(b.day,0)) as suitday
