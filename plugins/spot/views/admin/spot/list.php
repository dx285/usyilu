...
Ext.onReady(
    function () {
        ...
        $("#addbtn").click(function(){
            ...
            ST.Util.addTab('添加景点',SITEURL+'spot/admin/spot/add/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}parentkey/product/itemid/4',0);
        });
        ...
        window.product_store = Ext.create('Ext.data.Store', {
            ...
            proxy: {
                type: 'ajax',
                api: {
                    read: SITEURL+'spot/admin/spot/spot/action/read',  //读取数据的URL
                    update: SITEURL+'spot/admin/spot/spot/action/save',
                    destroy: SITEURL+'spot/admin/spot/spot/action/delete'
                },
                reader: {
                    type: 'json',   //获取数据的格式
                    root: 'lists',
            ...
            columns: [
                ...
                },
                {
                    ...
                    renderer: function (value, metadata, record) {
                        ...
                        {
                            var spotid = id;
                            var spotkind_url='spot/admin/spot/tickettype/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}parentkey/kind/itemid/6/spotid/'+spotid;
                            var addticket_url = 'spot/admin/spot/ticket_op/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}parentkey/product/itemid/4/action/add/spotid/'+spotid;
...
{
    var url = SITEURL+'spot/admin/spot/ticket/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}parentkey/product/itemid/4/spotid/'+id;
...
function goModify(id)
{
    ...
    var url = SITEURL+'spot/admin/spot/edit/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}parentkey/product/itemid/4/id/'+id;

    parent.window.addTab('修改-'+title,url,0);
}
//modSuit
function modSuit(ticketid,spotid)
{
    ...
    var title = record.get('title');
    var url = SITEURL+'spot/admin/spot/ticket_op/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}parentkey/product/itemid/4/action/edit/spotid/'+spotid+'/ticketid/'+ticketid;
