...
class Model_Line_Suit extends ORM
{
    ...
    {
            $suit = DB::select()->from('line_suit')->where('id','=',$suitId)->execute()->current();
    ...
    public static function suit_price($suitId,$useDate)
    {
        ...
        $arr = DB::select()->from('line_suit_price')->where('suitid','=',$suitId)->and_where('day','=',$time)->execute()->current();
