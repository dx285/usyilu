...
c.lineid,c.id,min(c.suitday) as suitday from(select a.lineid,a.id,max(ifnull(b.day,0)) as suitday from sline_line_suit a left join sline_line_suit_price b on a.id=b.suitid group by a.id) c group by c.lineid) d on a.id=d.lineid where $w $order  limit $start,$limit";
   ...
   */
    ...
    public function action_ajax_suitsave()
    {
        ...
        {
            $model->reload();
            $this->save_baojia($lineid, $model->id, $_POST);
            echo $model->id;
        }
        else
            echo 'no';
    ...
    public function save_baojia($lineid, $suitid, $arr)
    {
        ...
        $isdelete = empty($adultprice)&&empty($childprice)&&empty($oldprice)?true:false;
        //按日期范围报价
        if ($pricerule == 'all')
        {
            ...
            while (true)
            {
                ...
                if($isdelete)
                {
                    $query = DB::delete('line_suit_price')->where("suitid=$suitid and day='$begintime'");
                    $query->execute();
                }
        ...
        else if ($pricerule == 'month')
        {
            ...
            while (true)
            {
                ...
                foreach ($monthval as $v)
                {
                    ...
                    if($isdelete)
                    {
                        $query = DB::delete('line_suit_price')->where("suitid=$suitid and day='$newtime'");
                        $query->execute();
                    }
                    else if ($model->suitid)
                    {
    ...
    public function action_min_price_rule()
    {
        ...
        $minprice = Model_Sysconfig::get_configs(0,'cfg_line_minprice_rule',true);
