...
class Taglib_Line
{
    private static $basefiled = 'a.id,a.webid,a.aid,a.title,a.storeprice,a.litpic,a.startcity,a.kindlist,a.attrid,a.bookcount,a.storeprice,a.sellpoint,a.iconlist,a.satisfyscore';
    ...
    public static function line_spot($params)
    {
        ...
        foreach ($arr as &$r)
        {
            $s = "SELECT aid,webid FROM `sline_spot` WHERE id='{$r['spotid']}' and ishidden=0";
            $spot = self::execute($s);
            if(empty($spot))
                continue;
