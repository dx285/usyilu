...
<head>
<meta charset="utf-8">
<title>线路列表-思途CMS{$coreVersion}</title>
 ...
 <div class="search-bar filter" id="search_bar">
     ...
     <div class="change-btn-list mt-4  ml-10">
       <span class="change-btn-xz btnbox" id="attrlist" data-url="box/index/type/attrlist/typeid/1" data-result="result_attrlist" >属性&nbsp;&gt;&nbsp;<span id="result_attrlist">全部</span></span>
        ...
        <input type="text" id="searchkey" value="线路名称/线路编号/供应商" datadef="线路名称/线路编号/供应商" class="sty-txt1 set-text-xh wid_200">
...
<script>
  ...
  Ext.onReady(
    function() 
    {
        ...
        $("#addbtn").click(function(){
            ...
            ST.Util.addTab('添加线路',SITEURL+'line/admin/line/add/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}parentkey/product/itemid/1',0);
        });
					  ...
					  //  return  "<input type='checkbox' class='product_check' value='"+value+"'/>"; 
                                     ...
                                     var name=record.get('title');
			                          if(id.indexOf('suit')!=-1)
                                      {
									     ...
									     return "<a href='javascript:;' class='row-mod-btn' title='修改套餐'  onclick=\"modify_suit('"+id+"')\"></a>&nbsp;&nbsp;<a href='javascript:void(0);' title='删除' class='row-del-btn' onclick=\"delSuit('"+id+"')\"></a>";
                                      }
                                      else
                                      {
                                          return '<a href="javascript:;" class="row-add-suit-btn" onclick="ST.Util.addTab(\'添加套餐\',\'line/admin/line/addsuit/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}parentkey/product/itemid/1/lineid/'+id+'\')">添加套餐</a>';
                                      }
					},				 
				   listeners:{
					    afterrender:function(obj,eopts)
			   ...
			   },
			    {
				   ...
				   renderer : function(value, metadata,record) {
						 ...
						 return "<a href='javascript:void(0);' title='编辑' class='row-mod-btn' onclick=\"goModify("+id+")\"></a>"
                                +"&nbsp;&nbsp;<a href='javascript:void(0);' title='克隆' class='row-clone-btn' onclick=\"ST.Util.goClone("+id+",'line/admin/line')\"></a>";
                                   // return getExpandableImage(value, metadata,record);
                    },
					 listeners:{
					    afterrender:function(obj,eopts)
  ...
  function goModify(lineid)
  {
      ...
      parent.window.addTab('修改-'+title,SITEURL+'line/admin/line/edit/lineid/'+lineid+'/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}parentkey/product/itemid/1',1);
  }
  ...
  function modify_suit(id)
  {
      ...
      ST.Util.addTab('修改-'+title,'line/admin/line/editsuit/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}parentkey/product/itemid/1/suitid/'+suitid);
  }
