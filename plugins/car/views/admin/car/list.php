...
<script>
  ...
  Ext.onReady(
    function() 
    {
        ...
        $("#addbtn").click(function(){
            var url=SITEURL+"car/admin/car/add/parentkey/member/itemid/1";
            ST.Util.addTab('添加车辆',SITEURL+'car/admin/car/add/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}parentkey/product/itemid/3',0);
        });
        ...
        window.product_store=Ext.create('Ext.data.Store',{
         ...
         proxy:{
		   type:'ajax',
		   api: {
              read: SITEURL+'car/admin/car/car/action/read',  //读取数据的URL
			  update:SITEURL+'car/admin/car/car/action/save',
			  destroy:SITEURL+'car/admin/car/car/action/delete'
              },
		      reader:{
                type: 'json',   //获取数据的格式 
                root: 'lists',
			   ...
			   },
			   {
			   ...
			   },
			    {
				   ...
				   renderer : function(value, metadata,record) {
					     ...
					     var id=record.get('id');
						 return "<a href='javascript:void(0);'class='row-mod-btn' title='编辑' onclick=\"goModify("+id+")\"></a>&nbsp;&nbsp;"+"<a href='javascript:void(0);' title='克隆' class='row-clone-btn' onclick=\"ST.Util.goClone("+id+",'car')\"></a>";
                                   // return getExpandableImage(value, metadata,record);
                    },
					 listeners:{
					     afterrender:function(obj,eopts)
  ...
  function delSuit(id)
  {
    ...
    function goModify(id)
    {
        ...
        var url = SITEURL+'car/admin/car/edit/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}parentkey/product/itemid/3/carid/'+id;
    ...
    function modSuit(id)
    {
        var record=window.product_store.getById('suit_'+id);
        var title = record.get('title');
        var url = SITEURL+'car/admin/car/editsuit/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}suitid/'+id;
