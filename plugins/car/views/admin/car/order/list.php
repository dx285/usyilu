...
<head>
    <meta charset="utf-8">
    <title>租车订单管理-思途CMS{$coreVersion}</title>
    ...
    function () {
        ...
        window.product_store = Ext.create('Ext.data.Store', {
            ...
            proxy: {
                type: 'ajax',
                api: {
                    read: SITEURL+'car/admin/order/index/action/read/',  //读取数据的URL
                    update: SITEURL+'car/admin/order/index/action/save/',
                    destroy: SITEURL+'car/admin/order/index/action/delete/'
                },
                reader: {
                    type: 'json',   //获取数据的格式
                    root: 'lists',
...
$(function(){
    ...
    $(".btn_report").click(function(){
        var url=SITEURL+"car/admin/order/dataview/typeid/"+typeid;
        ST.Util.showBox(channelname+'订单数据报表查看',url,860,510,function(){});

    })
    //导出excel
    $(".btn_excel").click(function(){
        var url=SITEURL+"car/admin/order/excel/typeid/"+typeid;
        ST.Util.showBox(channelname+'订单生成excel',url,560,380,function(){});
    })

    //获取当前产品订单常规信息
    $.getJSON(SITEURL+'car/admin/order/ajax_sell_info/typeid/'+typeid,function(data){
...
function updateField(ele, id, field, value, type) {
    ...
    Ext.Ajax.request({
        url: SITEURL+"car/admin/order/index/action/update/",
        method: "POST",
        datatype: "JSON",
...
function view(id,typeid)
{
    ...
    var url=SITEURL+"car/admin/order/view/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}id/"+id+"/typeid/"+typeid;
