...
<script>
   ...
   // var otherModule={php echo json_encode($allmodule);};
        ...
        function () {
            ...
            window.dest_store = Ext.create('Ext.data.TreeStore', {
                ...
                proxy: {
                    type: 'ajax',
                    api: {
                        read: SITEURL+'article/admin/destination/destination/action/read',  //读取数据的URL
                        update: SITEURL+'article/admin/destination/destination/action/save',
                        destroy: SITEURL+'article/admin/destination/destination/action/delete'
                    },
                    reader: 'json'
                },
                autoLoad: true,
    ...
    function togStatus(obj, record, field) {
        ...
        Ext.Ajax.request({
            url: SITEURL+"article/admin/destination/destination/action/update",
            method: "POST",
            datatype: "JSON",
    ...
    function destProductSet(dest_id) {
        ...
        ST.Util.showBox('设置',SITEURL+'article/admin/destination/dialog_productinfo?id='+id+"&typeid="+display_mode,600,'',null,null,document,{loadCallback:setProductInfo,loadWindow:window});
    ...
    function searchDest() {
        ...
        if (!s_str)
            return;
        Ext.Ajax.request({
            url: SITEURL+'article/admin/destination/destination/action/search',
    ...
    function viewDestType(id){
        ...
        var url=BASEHOST+'/'+module_info['path']+'/'+pinyin;
        window.open(url);
