...
class Controller_Admin_Article  extends Stourweb_Controller{
...
b.isjian,ifnull(b.displayorder,999999) as displayorder,b.isding,a.modtime,a.templet from sline_article as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=4) where $w $order limit $start,$limit";
			...
			{
			   $sql="select a.id,a.aid,a.title,a.attrid,a.kindlist,a.finaldestid,a.webid,a.addtime,a.ishidden,a.themelist,a.iconlist,if(length(ifnull(a.attrid,''))=0,0,1) as order_attrid,if(length(ifnull(a.kindlist,''))=0,0,1) as order_kindlist,if(length(ifnull(a.iconlist,''))=0,0,1) as order_iconlist,if(length(ifnull(a.themelist,''))=0,0,1) as order_themelist,b.isjian,b.displayorder,b.isding,a.modtime,a.templet from sline_article as a left join sline_kindorderlist as b on (b.classid=$kindid and a.id=b.aid and b.typeid=4)  where $w $order limit $start,$limit";
    ...
    * */
    ...
    public function action_ajax_save()
    {
        ...
        {
            $model->litpic = self::dowload_picture($content);
        }
        $summary = Arr::get($_POST,'summary');

        $summary = empty($summary) && !empty($content)? mb_substr(strip_tags($content), 0, 140, 'utf-8').'...' : $summary;
    ...
    * */
     ...
     * 下载远程图片到本地
     * */
    private  function dowload_picture($content)
    {
