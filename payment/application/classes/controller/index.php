...
class Controller_Index extends Stourweb_Controller
{
    ...
    public function action_index()
    {
        ...
        //"0元"订单
        if ($view->info['total'] == 0 && $view->info['usejifen'])
        {
            Model_Member_Order::zero_pay($ordersn);
        }
        //编号
        $view->number = Common::product_number($view->info['productaid'], $view->info['typeid']);
        $view = str_replace(array('<stourweb_title/>', '<stourweb_pay_content/>'), array('确认订单', $view->render()), $this->_platFrom->content);
