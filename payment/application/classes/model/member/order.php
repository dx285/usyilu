...
class Model_Member_Order extends ORM
{
    ...
    public static function info($ordersn)
    {
        ...
        else
        {
            ...
            //是否使用优惠券
            if(Common::is_coupon_instal())
            {

                $info['iscoupon'] = Common::get_order_view($ordersn);
                if($info['iscoupon'])
                {
    ...
    {
        //更改处理中订单状态 status=1
        $rows = DB::update('member_order')->where('ordersn', '=', "{$ordersn}")->and_where('status', '=', 1)->set(array('status' => 2, 'paysource' => $payMethod))->execute();
        if ($rows == 1)
        {
            // 添加电子票
